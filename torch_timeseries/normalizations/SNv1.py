...
class SNv1(nn.Module):
    def __init__(self,  seq_len, pred_len, enc_in, sample_nums=8, element_wise_affine=True):
        super().__init__()
    ...
    def normalize(self, batch_x):
        ...
        Z = self.z_model(Q - mu.unsqueeze(1)) +  input_mu[:, -1:, :].unsqueeze(2) # (B C O N)
...
class MLP(nn.Module):
    ...
    def forward(self, x, x_norm=None):
        ...
        # x_norm = self.input_hat(x_norm)  # (B T O)
        # v = self.filter(x) + self.gate(x_norm)
