...
class CovidsV4(nn.Module):
    def __init__(self, seq_len, pred_len, enc_in, period_len=48):
        super().__init__()
...
# #         Sigma_ts = torch.einsum('btv,btn->bvn', output_center[:,i,:,:], output_center[:,i,:,:]) / (self.period_len - 1)
# #         output_Sigma_ts.append(Sigma_ts)
# #     return torch.stack(output_Sigma_ts, 1)
...
#         I = torch.eye(dim, device=U.device)
#         U_regularized = U + epsilon * I
#         # predict U2
#         # self.U2 = self.model_U2(U, Sigma, 'U2')
#         self.U2 = torch.linalg.inv(U_regularized)
...
#         #     Sigma_ts = torch.einsum('btv,btn->bvn', output_center[:,i,:,:], output_center[:,i,:,:]) / (self.period_len - 1)
#         #     output_Sigma_ts.append(Sigma_ts)
#         # self.output_Sigma = torch.stack(output_Sigma_ts, 1)
#         self.output_Sigma = torch.einsum('bmtv,bmtn->bmvn', output_center, output_center) / (self.period_len - 1) # (B, M, N, N)
#         return output.reshape(bs, len, dim) # (B,O, N)
...
#         if mode == 'U2':
#             self.final_activation = nn.Identity()
#             self.input_raw1 = nn.Linear(int(seq_len), 64)
#             self.input_raw2 = nn.Linear(int(seq_len), 64)
#             self.activation = nn.Tanh()
#             self.output = nn.Linear(128, self.pred_len) 
...
#             x = self.input_raw1(x)
#             x_raw2 = self.input_raw2(x_raw)
#             x = torch.cat([x, x_raw2], dim=-1)  # (B, N, N, 1024)
#             x = self.output(self.activation(x)) # (B, N, N, O)
#             x = self.final_activation(x) 
#             return x.permute(0, 3, 1, 2) # (B, O, N, N)
