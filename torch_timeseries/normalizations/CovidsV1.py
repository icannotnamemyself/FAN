...
class CovidsV1(nn.Module):
    def __init__(self,  seq_len, pred_len, enc_in, sample_nums=4):
        super().__init__()
    ...
    def normalize(self, batch_x):
        # (B, T, N)
        Q = self.upsample_layer(batch_x.unsqueeze(-1).permute(0, 3,1,2)) # (B, T, N)
        mu = Q.mean(dim=1) # (B, 1, T, N)
        std = Q.std(dim=1)

        input_mu = batch_x.mean(1, keepdim=True)

        x_norm = (batch_x - mu) / std
        # self.preds_mean = self.model( mu -  batch_x  , x_norm) * self.weight[0] + mu.mean(1, keepdim=True) * self.weight[1]
        self.preds_mean = self.model( mu -  input_mu  , x_norm) * self.weight[0] + input_mu * self.weight[1]
