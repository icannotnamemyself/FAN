...
def main_freq_part(x, k, rfft=False):
    # freq normalization
    # start = time.time()
    if rfft:
...
class PeriodFDV5(nn.Module):
    """Period FD V5 first substract bottom k frequecy component from the original series, then san
      

    Args:
        nn (_type_): _description_
    """
    def __init__(self,  seq_len, pred_len, enc_in, freq_topk = 20, period_len=12 , SAN=True, rfft=False):
        super().__init__()
