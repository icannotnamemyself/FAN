...
class MIC(nn.Module):
    ...
    def __init__(self, feature_size=512, n_heads=8, dropout=0.05, decomp_kernel=[32], conv_kernel=[24],
        ...
        self.decomp = nn.ModuleList([SeriesDecomp(k) for k in decomp_kernel])
...
class MICN(nn.Module):
    ...
    """
    def __init__(self, seq_len,pred_len,enc_in,c_out,d_model=512,freq='h',dropout=0.0,embed='timeF',d_layers=2,n_heads=8,num_class=0, conv_kernel=[12, 16],task_name="long_term_forecast"):
        """
        ...
        self.decomp_multi = SeriesDecompMulti(decomp_kernel)

        # embedding
        self.dec_embedding = DataEmbedding(enc_in, d_model, embed, freq,
                                           dropout)
        ...
        if self.task_name == 'classification':
            ...
            self.projection = nn.Linear(c_out * seq_len, num_class)
